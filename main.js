...
if (global.config.pairingAuth && !conn.authState.creds.registered) {
    ...
    if (!!global.config.pairingNumber) {
        ...
        if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code in config.js, Example : 62xxx")))
            process.exit(0)
        }
    } else {
...
async function filesInit() {
    const files = readdirSync(pluginFolder, { recursive: true })
    
    for (let filename of files) {
        if (!pluginFilter(filename)) continue
        try {
...
global.reload = async (_ev, filename) => {
    if (pluginFilter(filename)) {
        // Path lengkap file yang berubah
        let dir = global.__filename(join(pluginFolder, filename), true)
        
        // Cek apakah file masih ada (bukan dihapus)

    ...
    switch (action) {
        case 'add': {
            ...
            for (let user of participants) {
                ...
                let p = groupMetadata?.participants?.find(x => x.id === user || x.lid === user || x.jid === user)
                let realJid = p?.jid || user
                text = (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user')
                text = text.replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow').replace('@user', "@" + realJid.split("@")[0])
                // ==========================================================
